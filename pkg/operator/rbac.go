...
func VerifyRole(clients *clients.Clients, role, namespace string) {
	err := wait.PollUntilContextTimeout(clients.Ctx, config.APIRetry, config.APITimeout, false, func(context.Context) (done bool, err error) {
		log.Printf("Verifying that role %s exists in namespace %s\n", role, namespace)
		r, err := clients.KubeClient.Kube.RbacV1().Roles(namespace).Get(context.TODO(), role, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		if r.Name == role {
	...
	if err != nil {
		testsuit.T.Fail(fmt.Errorf("expected: role %q present in namespace %q, Actual: role %q not present in namespace %q, Error: %v", role, namespace, role, namespace, err))
	}
}
